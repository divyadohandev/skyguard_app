...
Spatially inconsistent observations

Potential regional weather extremes
...
Real IMD AWS historical data is not yet integrated.

Peer-station information is prototype/hardcoded.

Spatial checking currently focuses on temperature for the configured prototype station.

Temporal state is stored in memory.

Restarting the backend resets previous-station state.

There is no production database.

Confidence is not calibrated probability.

Sensor health is a prototype composite indicator.

Self-healing is data estimation, not physical repair.

Fault classification is currently rule-based.

sequence_id is accepted but not yet used as a decision feature.

Production requires authentication, persistence, monitoring, logging, rate limiting and secure secret management.
...
Previous-vs-current reading check
...
Prevent suspicious data from normal use
